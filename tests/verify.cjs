...
// New badges credit distinct successful tasks, including existing mastery.
dom=load();w=dom.window;assert.equal(ev('earnedBadges().length'),12);assert.equal(ev('earnedBadges().filter(b=>b.earned).length'),0);
for(const [type,name,goal] of [['order','Puzzleprofi',5],['error','Spürnase',5],['input','Wortakrobat',10],['choice','Treffsicher',10]]){
 ev('state=defaults()');const ids=ev(`D.questions.filter(q=>q.type==='${type}').slice(0,${goal}).map(q=>q.id)`);
 for(let i=0;i<ids.length;i++){ev(`state.answers['${ids[i]}']={correct:false,everCorrect:true,attempts:20}`);assert.equal(ev(`earnedBadges().find(b=>b.name==='${name}').earned`),i+1===goal);}
}
ev('state=defaults();for(const type of ["input","choice","order","error"]){const q=D.questions.find(q=>q.type===type);state.answers[q.id]={correct:true};}');assert(ev('earnedBadges().find(b=>b.name==="Rundum fit").earned'));
ev('state=defaults();for(const q of D.questions.filter(q=>D.grammar.slice(0,7).some(t=>t.id===q.topic)))state.answers[q.id]={correct:false,firstCorrect:true};');assert(ev('earnedBadges().find(b=>b.name==="Themenchampion").earned'));assert(ev('earnedBadges().find(b=>b.name==="Sternenreisender").earned'));assert(!ev('earnedBadges().find(b=>b.name==="Milos Lernheld").earned'));
ev('for(const q of D.questions)state.answers[q.id]={correct:true};');assert(ev('earnedBadges().find(b=>b.name==="Milos Lernheld").earned'));route('progress');assert.equal(w.document.querySelectorAll('.badge-list li').length,12);assert(w.document.querySelector('.badge-list').textContent.includes('Löse 5 verschiedene Satzpuzzles.'));dom.window.close();
